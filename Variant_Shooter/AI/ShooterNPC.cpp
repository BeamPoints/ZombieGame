...
void AShooterNPC::AttachWeaponMeshes(AShooterWeapon* WeaponToAttach)
{
	...
	WeaponToAttach->GetFirstPersonMesh()->AttachToComponent(GetFaceMesh(), AttachmentRule, FirstPersonWeaponSocket);
...
FVector AShooterNPC::GetWeaponTargetLocation()
{
	...
	const FVector AimSource = GetFaceMesh()->GetComponentLocation();
	...
	if (CurrentAimTarget)
	{
		...
		AimDir = UKismetMathLibrary::RandomUnitVectorInConeInDegrees(GetFaceMesh()->GetForwardVector(), AimVarianceHalfAngle);
